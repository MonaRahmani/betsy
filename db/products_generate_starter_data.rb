...
CSV.open("db/products_seeds.csv", "w", :write_headers => true,
         :headers => ["username", "name", "price", "stock", "categories", "description", "photo_url", "retired"]) do |csv|
  20.times do
    username = ""
    name = ""
    description = ""
    ...
    csv << [username, name, price, stock, categories, description, photo_url, retired]
  end
end
